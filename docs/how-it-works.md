...
cc-mem はプロジェクトスコープ機能を持ち、検索とリコールをデフォルトで現在のプロジェクトに限定します。
...
- `cc-mem recall` -- `my-app` プロジェクトの直近セッションを表示
- `cc-mem recall --all` -- 全プロジェクトの直近セッションを表示
