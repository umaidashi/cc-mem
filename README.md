...
- **Project scope** — 検索・リコールは自動的に現在のプロジェクトにスコープ。`--all` で横断検索も可能
...
Session starts (SessionStart Hook)
  │
  └─ Recall:  直近3セッションの概要を自動表示（現プロジェクトにスコープ）
...
| `CC_MEM_PROJECT` | CWD の basename | プロジェクトスコープ名（search/recall のデフォルトスコープ） |
